...
__inline__ void
list_ctrl_push_front(t_list_ctrl* list, t_list* node)
{
  list->head = list_push_front(list->head, node);
}

__inline__ void
list_ctrl_push_back(t_list_ctrl* list, t_list* node)
{
  if (!list->head) {
    node->prev = NULL;
    node->next = NULL;
...
__inline__ void
list_ctrl_pop_front(t_list_ctrl* list, t_free_func func)
{
  t_list* tmp;
...
__inline__ void
list_ctrl_pop_back(t_list_ctrl* list, t_free_func func)
{
  t_list* tmp;
...
__inline__ size_t
list_ctrl_len(t_list_ctrl* list)
{
  int i;
  t_list* tmp;
...
void
list_ctrl_free(t_list_ctrl* controler, t_free_func func)
{
