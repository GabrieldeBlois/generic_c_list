...
__inline__ static
t_list_sort *new_tree_node(t_list *node)
{
    t_list_sort *tmp;
...
static t_list_sort *add_to_tree(t_list_sort *root, t_list *node, t_comparison_func func)
{
    if (!root)
        return new_tree_node(node);
    else if (func(root->node->data, node->data) <= 0)
        root->left = add_to_tree(root->left, node, func);
    else
        root->right = add_to_tree(root->right, node, func);
    return (root);    
}
...
t_list *list_sort(t_list *list, t_comparison_func func)
{
    ...
    while (list)
    {
        root = add_to_tree(root, list, func);
        list = list->next;
    }
